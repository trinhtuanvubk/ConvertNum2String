...
def wrong_special_datetime(datetime:str):
...
def check_special_datetime_case(datetime:str):
    # NOTE: check in the case datetime is month-year but the output is day-month (year->day,month->month)
    d_m_y =  re.split('\_|\-|\/|\|',datetime)
...
def replace_date_string(datetime:str):
    ...
    text = ""
    if wrong_special_datetime(datetime):
        ...
        return text 
    if check_special_datetime_case(datetime):
...
def datetime_to_string(paragraph:str): 
    ...
    text = text.lower()
    text = re.sub('\s{2,}', ' ', text) # remove space 
...
if __name__ == "__main__": 
    ...
    paragraph = datetime_to_string(example1) 
    print(paragraph)
